    ...
    def train_step(self, data):
        ...
        for i in range(self.d_steps):
            ...
            with tf.GradientTape() as d_tape:
                ...
                # Watch the gradients
                # print("\n \n Trainable Weights:", self.discriminator.trainable_weights)
                # tensorflow_variables = [tf.Variable(v, trainable=True) for v in self.discriminator.trainable_weights]
...
def compute_tstr(meta_data, model, d_steps, NOISE_DIM):
    ...
    history = tstr_regressor.fit(x_train, y_train,  validation_data=(x_val,y_val), epochs=500, batch_size=32, callbacks=[early_stop]) 
